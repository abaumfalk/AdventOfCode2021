...
class SnailfishNumber(Visitable):
    ...
    def __add__(self, other: 'SnailfishNumber'):
        # recreate from list to avoid modifications of the originals
        left = SnailfishNumber.from_list(self.to_list())
        right = SnailfishNumber.from_list(other.to_list())
...
if __name__ == "__main__":
    ...
    snailfish_numbers = [SnailfishNumber.from_string(line) for line in lines]

    max_magnitude = 0
    for first in snailfish_numbers:
        for second in snailfish_numbers:
            if first == second:
                continue
            result = first + second
